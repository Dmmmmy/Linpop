...
void setup_tree_view(GtkWidget *treeview)
{
	...
	// 仅文本 column
	renderer = gtk_cell_renderer_text_new();
	column = gtk_tree_view_column_new_with_attributes("IP地址", renderer, 
			"text", IP, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW (treeview), column);
}
		...
		gchar *my_id)
{
	...
	GtkTreePath *path_expand = NULL; // 只打开自己和所在treeview
	guint i, j, on, all;
	GdkPixbuf *pixbuf;
	...
	if (array_user != NULL)
		for (i = 0; i < array_user->len; i++)
		{
			...
			{
				// 增加用户信息子节点
				gchar *gstrface = g_strdup_printf("./mnt/hgfs/Image/tx/%d.bmp",
						pItem->face);
